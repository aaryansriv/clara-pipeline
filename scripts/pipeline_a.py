...
Usage: python pipeline_a.py --transcript path/to/transcript.txt --account-id ACC001
       python pipeline_a.py --transcript path/to/recording.m4a --account-id ACC001
"""
...
from schema import (
    AccountMemo, BusinessHours, RoutingRule, CallTransferRules,
    PricingInfo, NotificationChannels, slugify
)
from extractor import extract_from_transcript, transcribe_audio_if_needed
...
def build_account_memo(extracted: dict, account_id: str) -> AccountMemo:
    ...
    # Build PricingInfo if present
    pi_data = extracted.get("pricing_info") or {}
    pricing_info = PricingInfo.from_dict(pi_data) if pi_data else None

    # Build NotificationChannels if present
    nc_data = extracted.get("notification_channels") or {}
    notification_channels = NotificationChannels.from_dict(nc_data) if nc_data else None
...
def run_pipeline_a(transcript_path: str, account_id: str = None):
    ...
    # Step 0: Handle audio transcription if needed
    logger.info("Step 0: Loading input (auto-detecting audio vs text)...")
